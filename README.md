...
> README описывает объединённую сборку **`main`**, API **0.3.0**. Конструктор в браузере сохраняет описание, вопросы, ответы и рабочую карточку. Подтверждение, рейтинг и публикация пока доступны через API и Swagger. Каталог и полная страница задачи используют явно обозначенные демонстрационные примеры.
...
| №3 — интерфейс команды | Шаги 1–3: каталог примеров, полная страница, форма с локальными черновиками | Подключение отправки к API и кабинет команды |
...
- **Готовить черновик отклика.** На странице задачи команда заполняет идею, план, сроки, необязательную ссылку на прототип и вопросы. Черновики сохраняются в браузере отдельно для каждой команды и задачи; отправка бизнесу пока отключена.
...
- **Следующие шаги интерфейса ещё не выполнены.** Конструктор на `/ui` работает до сохранения рабочей карточки. Шаг 3 участника №2 (оценка, подтверждение и публикация в UI) пока не подключён. Форма отклика участника №3 сохраняет только локальные черновики; серверную отправку ещё предстоит подключить. Кабинеты бизнеса и команды — заглушки.
