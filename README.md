...
> README описывает текущую сборку, API **0.3.0**. Конструктор в браузере сохраняет описание, вопросы, ответы и рабочую карточку, показывает серверный рейтинг и позволяет вручную подтвердить и опубликовать задачу. Каталог и полная страница задачи используют явно обозначенные демонстрационные примеры.
...
- **Кабинеты и серверная отправка отклика ещё не выполнены.** Конструктор на `/ui` работает до подтверждения и публикации с серверным рейтингом. Форма отклика участника №3 сохраняет только локальные черновики; серверную отправку ещё предстоит подключить. Кабинеты бизнеса и команды — заглушки.
