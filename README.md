...
> Ветка участника №3 завершает интерфейс команды: публичный каталог, отклики, результаты этапов и подтверждённые баллы. Для этих возможностей нужен **API v0.4 из `feat/backend-workflow`**. Совместимость проверена на `380d94a` в отдельной сборке; серверные файлы этой frontend-ветки пока остаются v0.3. Финальная синхронизация всех участников в main ещё не выполнена. [Порядок сборки и отчёт](docs/TEAM_WORKSPACE.md).

| Участник | Объединено и проверено | Следующий шаг |
|---|---|---|
| №1 — backend | Шаги 1–3 в основе; API шага 4 проверен в отдельной ветке | Включить `feat/backend-workflow` при синхронизации |
...
| №3 — интерфейс команды | Каталог API, карточка, отправка отклика, кабинет, результаты этапов и баллы | Общая синхронизация и финальное демо |
...
- **Находить опубликованные задачи.** Каталог Soft Retro использует публичный API v0.4: поиск, фильтры, сортировка и полная карточка. Возврат сохраняет фильтры; ошибки не подменяются примерами.
- **Предлагать решение.** Команда заполняет идею, план, сроки, необязательные прототип и вопросы. Черновик сохраняется локально; отдельное действие отправляет его в API. Повторный отклик на ту же задачу заменяется показом существующего статуса.
- **Следить за работой команды.** «Мои отклики» показывает решения бизнеса, позволяет выбранной команде сдавать этапы и отображает баллы только после серверного подтверждения бизнесом. Эти возможности требуют API v0.4.
...
С API v0.4 публикация появляется в общем каталоге. Команда отправляет предложение,
бизнес вручную выбирает исполнителей, выбранные команды сдают результаты этапов.
Подтверждение этапа бизнесом даёт 10 командных баллов; повторное подтверждение
не удваивает начисление. Кабинет бизнеса подключает участник №2; до этого решения
можно проверить через Swagger. [Сценарий команды](docs/TEAM_WORKSPACE.md).
...
flowchart LR
    ...
    Constructor --> API
    Browser --> Team[Каталог и кабинет команды]
    Team --> Workflow[API v0.4: backend-workflow]
    Workflow --> DB
```
...
app/
  ...
  static/            # Стили, JavaScript, иконки; каталог и кабинет через API
...
В БД предусмотрены бизнес-профили, команды, задачи, отклики и этапы. Задача хранит
исходный текст, рабочую карточку, подтверждённый и опубликованный снимки отдельно.
Операции с откликами и этапами подключены в зависимой backend-ветке v0.4.
...
Основные шаги 1–3 находятся в `main`. До финальной синхронизации для полного пути
команды нужна согласованная сборка `feat/backend-workflow` и `feat/team-proposal-form`;
не заменяйте чужие изменения целыми файлами. [Инструкция интеграции](docs/TEAM_WORKSPACE.md).
После обновления перезапустите сервер; существующую БД не удаляйте.
...
- [Интерфейс](http://127.0.0.1:8000/ui) — конструктор; с API v0.4 также каталог и кабинет команды.
...
действия. С API v0.4 публикация появится в каталоге: перейдите туда или нажмите «Обновить».
...
Интерфейс команды дополнительно проверен с backend v0.4: **116 pytest-тестов**
в совместной временной сборке и **5 тестов JavaScript** командой
`node --test tests/team_api.test.cjs` (Node нужен только для этой проверки).
В браузере пройден путь отклик → выбор команды → результат → подтверждённые баллы,
а также сохранение ввода и повтор отправки после обрыва соединения.
Подробный отчёт: [TEAM_WORKSPACE.md](docs/TEAM_WORKSPACE.md).
...
| [`app/static/catalog-demo.json`](app/static/catalog-demo.json) | Историческая синтетическая тестовая фикстура; текущий UI её не загружает |
...
- **Общая синхронизация ещё впереди.** Интерфейс команды требует зависимую ветку `feat/backend-workflow` (v0.4); без неё API каталога/откликов вернёт 404. Кабинет бизнеса остаётся ответственностью участника №2.
- **Начальный каталог пуст.** Seed создаёт черновики: их нужно явно подтвердить и опубликовать. Данные обновляются по входу в раздел и кнопкой, без фонового опроса.
- **Отклик нельзя редактировать или отозвать после отправки.** Текущий API не предусматривает эти операции. Черновики хранятся на текущем устройстве; при запрете хранилища — только в памяти вкладки.
...
- [Форма отклика и локальные черновики](docs/PROPOSAL_FORM.md)
- [Завершённый кабинет команды, проверка и синхронизация](docs/TEAM_WORKSPACE.md)
