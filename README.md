...
> Основная инструкция описывает **`main`, API 0.3.0**. В этой сборке бизнес проходит путь от описания до подтверждения и публикации через браузер. Каталог показывает демонстрационные примеры. Готовые расширения в отдельных ветках перечислены в разделе «Ограничения и состояние интеграции».

**Дополнение в `feat/business-ui`:** кабинет на `/ui#business` показывает реальные задачи и открывает их в конструкторе. Сравнение предложений, выбор/отклонение команд и подтверждение этапов проверены с настоящим API из `feat/backend-workflow` (`17678d0`) в изолированной сборке. На API 0.3 интерфейс явно сообщает об отсутствии операций откликов. Изменения пока не объединены с main. [Проверка и передача интеграции](docs/BUSINESS_DASHBOARD.md).

## Что реализовано

| Возможность | Как работает в `main` |
|---|---|
...
Публикация не требует минимального рейтинга. Даже задача с одним названием и **0 баллов** может быть опубликована. Новые публикации из SQLite пока не появляются в браузерном каталоге: его источник в `main` — отдельный демонстрационный JSON.
...
В схеме БД предусмотрены бизнес-профили, команды, задачи, отклики и этапы. В `main` операции с откликами и этапами ещё не подключены к API; готовая серверная реализация находится в отдельной ветке.
...
- **Общий каталог ещё не подключён к публикациям в `main`.** Каталог и полная страница читают статический набор; публичные API каталога, отклики, решения и этапы отсутствуют в этой версии основной ветки. Кабинеты бизнеса и команды остаются заглушками.
...
| [`feat/team-proposal-form`](https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team/tree/feat/team-proposal-form) | Форма предложения с проверкой полей и локальными черновиками; отправка на сервер отключена — [описание](https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team/blob/feat/team-proposal-form/docs/PROPOSAL_FORM.md) |
