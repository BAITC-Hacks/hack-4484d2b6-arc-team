...
> README описывает сборку **`feat/backend-scoring`**, API **0.3.0**. Основной сценарий подготовки и публикации задачи работает через API и интерактивную документацию Swagger. Интерфейс этой ветки содержит оболочку и демонстрационный каталог; границы текущей версии указаны ниже.
...
- **Демонстрировать поиск задач.** Каталог в стиле Soft Retro поддерживает поиск, фильтры по теме и готовности, сортировку по рейтингу и краткий просмотр карточки на синтетических примерах.
...
```sh
git clone --branch feat/backend-scoring https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team.git
cd hack-4484d2b6-arc-team
```

Ветка указана явно, поскольку описанные функции ещё не объединены с `main`.
...
- [Интерфейс](http://127.0.0.1:8000/ui) — оболочка и каталог примеров.
...
Набор включает **92 теста**: сохранность данных, миграции, изоляцию снимков, ошибки AI, веса и границы рейтинга, публикацию с 0 баллов, конфликты версий и повтор действий. Тесты используют временные базы и имитацию внешнего API; ключ OpenAI и платные запросы не нужны.
...
- **Интеграция интерфейса не завершена в этой ветке.** Конструктор на `/ui` — предварительный макет; серверный сценарий выполняется через Swagger. Действующий конструктор бизнеса присутствует отдельно в `feat/business-ui`, полная страница задачи — в `feat/team-task-detail`; эти изменения не входят в описанную сборку.
