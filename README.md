...
> Инструкция описывает **текущую ветку `feat/business-ui`, сервер API 0.3.0**. В ней работают конструктор, подтверждение и публикация, список задач бизнеса и переход к редактированию. Каталог пока показывает демонстрационные примеры. Сервер откликов и интерфейс команды подготовлены в отдельных ветках, но ещё не включены в эту сборку и `main`.

Сравнение предложений, выбор/отклонение команд и подтверждение этапов в кабинете проверены с настоящим API из `feat/backend-workflow` (`17678d0`) в изолированной сборке. При обычном запуске этой ветки интерфейс явно сообщает, что отклики ещё не подключены. Полный совместный браузерный прогон всех трёх частей остаётся этапом интеграции. [Проверка и передача интеграции](docs/BUSINESS_DASHBOARD.md).

## Что реализовано

| Возможность | Как работает в текущей ветке |
|---|---|
...
| Кабинет бизнеса | Реальные задачи выбранного владельца, фильтр публикации, подтверждённый рейтинг, открытие задачи и создание новой через конструктор |
...
Публикация не требует минимального рейтинга. Даже задача с одним названием и **0 баллов** может быть опубликована. Новые публикации видны в кабинете бизнеса, но пока не появляются в браузерном каталоге этой ветки: его источник — отдельный демонстрационный JSON.
...
6. В «Моих задачах» бизнес видит сохранённые задачи, статус публикации и подтверждённый рейтинг; может вернуться к редактированию.

Продолжение **отклик команды → сравнение → ручной выбор → результат этапа → подтверждение и баллы** реализовано частями в трёх ветках. Для доступности всего пути в одном приложении нужно их согласованное объединение; наличие экранов само по себе не подключает отсутствующий API.
...
Версии зафиксированы в [requirements.txt](requirements.txt) и [requirements-dev.txt](requirements-dev.txt). `gpt-4o-mini` — значение настройки по умолчанию, а не подтверждение доступа в конкретном аккаунте. Доступность модели проверяется при реальном вызове. Для приложения не требуются Node.js, сборка фронтенда и отдельный сервер базы данных. Собственная языковая модель не обучается.
...
flowchart LR
    ...
    Dashboard[Кабинет бизнеса: список задач] --> API
...
В схеме БД предусмотрены бизнес-профили, команды, задачи, отклики и этапы. На сервере API 0.3 этой ветки операции с откликами и этапами ещё не подключены; готовая серверная реализация находится в `feat/backend-workflow`. Кабинет передаёт выбранный бизнес через `X-Demo-Business-Id`; проверку принадлежности данных выполняет сервер.
...
```sh
git clone --branch feat/business-ui https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team.git
cd hack-4484d2b6-arc-team
```

Команда получает именно описанную здесь версию с кабинетом бизнеса. Для основной объединённой сборки замените `feat/business-ui` на `main`; в проверенном main (`4c827a2`) кабинет бизнеса ещё отсутствует. Клонирование отдельной ветки интерфейса не объединяет её с серверной веткой автоматически.
...
- [Кабинет бизнеса](http://127.0.0.1:8000/ui#business) — сохранённые задачи, фильтр публикации и переход к редактированию.
...
2. Введите: **«Остатки товаров ведём вручную. Хотим находить дефицит.»** Выберите тему **«Торговля»** (`retail`) и нажмите **«Получить вопросы»**.
...
8. Обновите публикацию отдельной кнопкой. Перейдите в **«Мои задачи»**, выберите фильтр **«Опубликованы»** и найдите созданную задачу с 70 баллами. Откройте её в конструкторе и перезагрузите страницу: данные и подтверждённый рейтинг сохраняются.
...
На текущем сервере кабинет показывает **«Отклики ещё не подключены»** — это ожидаемый результат, а не отсутствие интереса команд. Проверка предложений и этапов с API 0.4 описана в [документации кабинета](docs/BUSINESS_DASHBOARD.md). Она проводилась в отдельной сборке; отправка от имени команд выполнялась через API.
...
В текущей ветке сохранён набор из **94 серверных тестов**: хранение и миграции, ошибки AI, сохранение ответов, формула и границы рейтинга, публикация с 0 баллов, конфликты версий, защита опубликованных снимков и ресурсы интерфейса. Тесты используют временные базы, локальный режим и имитацию провайдера; платные AI-запросы не выполняются.
...
Проверка кабинета — `tests/browser_business.py`. На API 0.3 она явно использует тестовую HTTP-фикстуру для отсутствующих операций, а на API 0.4 выполняет реальный сценарий из `tests/browser_business_workflow.py`. Фикстура не входит в приложение и не доказывает работоспособность серверных операций. Проверка с настоящим backend-workflow, а также регрессия конструктора и 116 тестов серверной ветки выполнены отдельно; [подробности](docs/BUSINESS_DASHBOARD.md).
...
- **Единая сборка всех частей ещё не выполнена.** В текущей ветке каталог и полная страница читают статический набор. API откликов, решений и этапов отсутствует; кабинет бизнеса работает со списком задач, а эти операции явно недоступны. Кабинет команды пока остаётся заглушкой. Готовые расширения перечислены ниже.
...
В репозитории уже есть расширения, которые **ещё не объединены с `main`**:

| Ветка | Подтверждённый результат |
|---|---|
| [`feat/backend-workflow`](https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team/tree/feat/backend-workflow) | API 0.4.0: публичный каталог, отклики, выбор нескольких команд, результаты этапов, однократные 10 баллов за подтверждение; 116 тестов — [отчёт](https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team/blob/feat/backend-workflow/docs/QA.md) |
| [`feat/business-ui`](https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team/tree/feat/business-ui) | Шаг 4 участника №2: задачи бизнеса, сравнение откликов, ручной выбор/отказ, подтверждение этапов. Браузерная проверка с backend-workflow пройдена; серверная ветка ещё не включена — [описание](docs/BUSINESS_DASHBOARD.md) |
| [`feat/team-proposal-form`](https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team/tree/feat/team-proposal-form) | На проверенном коммите `85902e7`: публичный каталог, отправка предложений, «Мои отклики», отправка этапа и отображение серверных баллов; требуется API 0.4 — [описание](https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team/blob/feat/team-proposal-form/docs/TEAM_WORKSPACE.md) |

Эти результаты не означают, что полный путь команды уже доступен через интерфейс `main`. Для него требуется объединить и подключить соответствующие части.
...
- [Кабинет бизнеса: проверка и интеграция](docs/BUSINESS_DASHBOARD.md)
