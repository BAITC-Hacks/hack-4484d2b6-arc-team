...
> README описывает ветку **`feat/backend-workflow`**, API **0.4.0**. Сервер выполняет путь от черновика до отклика, выбора команды и подтверждения этапа с баллами. Конструктор браузера подключён до сохранения рабочей карточки; остальные новые операции доступны через Swagger и готовы к подключению интерфейсами.

| Участник | Объединено и проверено | Следующий шаг |
|---|---|---|
| №1 — backend | Шаги 1–5: хранение, AI, рейтинг, публикация, каталог, отклики, этапы и проверки | Совместный прогон через интерфейсы |
| №2 — интерфейс бизнеса | Шаги 1–2: Soft Retro и работающий конструктор | Шаг 3: панель рейтинга и кнопки подтверждения/публикации |
...
- **Принимать отклики и результаты.** Команды отправляют идею, план, срок и необязательную ссылку на прототип. Бизнес вручную выбирает одну или несколько команд. Подтверждённый результат этапа приносит команде 10 баллов без повторного начисления.
...
- **Работать с конструктором в браузере.** Выбор бизнеса, сохранение черновиков, вопросы, ответы и редактирование карточки подключены к API. Несохранённые правки восстанавливаются отдельно для каждого бизнеса и задачи.
...
**Описание → вопросы → карточка → подтверждение и рейтинг → публикация → отклик → выбор команды → результат этапа → подтверждение и баллы.**
...
6. Команда читает опубликованную задачу через публичный API и отправляет предложение. Бизнес вручную выбирает команды, остальные отклики не отклоняются автоматически.
7. Выбранная команда отправляет результат этапа. После подтверждения бизнеса начисляются 10 баллов; повторный запрос не увеличивает сумму.

Публичные API каталога и задачи читают опубликованные снимки из БД. Браузерный каталог пока использует демонстрационный набор; его подключает участник №3.
...
В БД предусмотрены бизнес-профили, команды, задачи, отклики и этапы. Отклики и этапы доступны через API; сумма баллов команды вычисляется из подтверждённых этапов. Задача хранит исходный текст, рабочую карточку, подтверждённый и опубликованный снимки отдельно.
...
```sh
git clone --branch feat/backend-workflow https://github.com/BAITC-Hacks/hack-4484d2b6-arc-team.git
cd hack-4484d2b6-arc-team
```

Базовые шаги команды объединены в `main`; серверный шаг 4 добавлен в `feat/backend-workflow`. Если продолжаете работу в своей ветке,
...
Кнопки подтверждения и публикации пока отключены — это следующий шаг участника №2.
Операции можно проверить через Swagger по сценарию ниже.
...
### Продолжение: отклик и результат команды

После публикации используйте ID созданной задачи:

1. `GET /api/catalog/{id}` без профиля — проверьте опубликованную карточку.
2. `POST /api/tasks/{id}/proposals`, заголовок `X-Demo-Team-Id: team-1`, тело:
   `{"idea":"Прототип учёта","plan":"Загрузить CSV и показать дефицит","timeline":"Две недели","prototype_url":null}`.
3. `GET /api/tasks/{id}/proposals` с `X-Demo-Business-Id: business-1` — найдите отклик.
4. `PATCH /api/proposals/{proposal_id}` от бизнеса, тело `{"status":"selected"}`.
5. `POST /api/proposals/{proposal_id}/milestones` от `team-1`, тело
   `{"description":"Прототип проверен на тестовом CSV","result_url":"https://example.test/demo"}`.
6. `POST /api/milestones/{milestone_id}/confirm` от бизнеса, без тела — получите 10 баллов.
7. Повторите подтверждение: время и баллы не изменятся. В `GET /api/my/proposals`
   от `team-1` виден подтверждённый этап; `/api/demo/profiles` показывает `teams[].points`.

Выбор нескольких команд и отклик на задачу с 0 баллов разрешены. Контракт и
[готовые JSON-примеры](docs/workflow-examples.json) позволяют подключить оба кабинета.
...
В проверке шага 5 прошли **116 тестов**: сохранность данных, миграции, изоляция снимков, ошибки AI, веса и границы рейтинга, публикация с 0 баллов, конфликты версий, повтор действий, ресурсы обеих страниц, сквозной серверный сценарий, изоляция откликов, выбор нескольких команд и одновременные повторы отправки/подтверждения этапов. Тесты используют временные базы и имитацию внешнего API; ключ OpenAI и платные запросы не нужны.
...
- **Следующие шаги интерфейса ещё не выполнены.** Конструктор на `/ui` работает до сохранения рабочей карточки. Шаг 3 участника №2 (оценка, подтверждение и публикация в UI) и шаг 3 участника №3 (форма отклика) намеренно оставлены для дальнейшей разработки. Кабинеты бизнеса и команды — заглушки.
- **Браузерный каталог использует примеры.** Публичные API `/api/catalog` и `/api/catalog/{id}` уже читают реальные публикации; подключение страниц к ним остаётся участнику №3.
- **Минимальный процесс работы.** Одна команда отправляет одно предложение на задачу. Изменение/отзыв предложения, отмена принятого решения, редактирование этапов и полный трекер проектов не предусмотрены. Каталог возвращает весь список без пагинации.
...
- [Результаты проверки сервера — шаг 5](docs/QA.md)
